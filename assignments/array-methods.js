...
let ticketPriceTotal2 = runners.reduce((acc, curr) => acc + curr.donation, 0);
...
// Problem 1
// Filter runners based on if their donation was over or under 100
let bigSpenders = runners.filter(runner => runner.donation >= 100);

console.log(bigSpenders);
// Problem 2

// Problem 3
// Subtract shirt cost from donation then get total profits
...
let shirtsNeeded = runners.map(runner => runner.shirt_size);
let shirtCosts = [];

shirtsNeeded.forEach(shirt => {
  shirtPrices.forEach(price => {
    if (shirt === price.size) {
      shirtCosts.push(price.cost);
    }
  });
});

let totalCost = shirtCosts.reduce((acc, curr) => acc + curr);
